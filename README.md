...
- 🔖 **书签/断点续播** - 保存和恢复播放位置
- 📝 **歌词显示** - 支持LRC格式歌词同步显示
- 🌐 **在线歌词搜索** - 播放时自动从QQ音乐/网易云聚合搜索歌词，按 `N` 手动搜索并选择候选，成功后缓存为同名 `.lrc`
...
播放音频时，若本地无同名 `.lrc` 文件，会**自动后台搜索**在线歌词（QQ音乐优先，网易云回退），成功后缓存为同名 `.lrc`，下次播放直接读取本地。

```bash
mp song.mp3                  # 本地无.lrc时自动在线搜索并缓存
                              # 状态栏会显示 📝 歌词(QQ) 或 📝 歌词(网易) 标识来源
```

播放中按 `N` 键**手动搜索**，可自定义关键词并从候选列表中选择：
...
**搜索策略**：
- **关键词构造**：优先使用元数据（`title` + `artist`），无元数据时回退到清洗后的文件名（去除序号、音质标识、网址、`_-` 分隔符等）
- **歌词源**：QQ音乐（原唱匹配好、时间轴完整）→ 网易云音乐（回退）
- **质量过滤**：仅采用带时间轴的 LRC 歌词，纯文本歌词自动丢弃
- **缓存策略**：搜索成功后自动保存为同名 `.lrc`；自动搜索不覆盖已存在的本地文件，手动搜索会覆盖
...
- 🛡️ 所有行截断到 `term_width - 1`，留 1 列余量防止内容恰好填满一行时某些终端自动换行
- 🔄 新增终端宽度变化检测 - 用户调整窗口大小时清除旧内容并重置行数计数，避免宽度变化导致行数错乱

### v2.11.4
- 🐛 修复找不到歌词 - 纯音乐占位歌词（`[00:00:00]此歌曲为没有填词的纯音乐`）含时间轴通过过滤，导致选中只有1行的无效歌词；`_has_timeline` 新增纯音乐/无歌词占位文本检测 + 有效歌词行数 >= 3 的下限
- 🎯 改进歌词选择策略 - 原取首个带时间轴的候选，可能选到只有几行的占位歌词；改为收集所有有效候选的歌词行数，选行数最多的（>= 15 行即停止尝试，减少网络请求）
- ➕ 新增酷狗音乐歌词源 - 三源聚合（QQ音乐 → 网易云 → 酷狗），提高搜索覆盖率，状态栏显示 `📝 歌词(酷狗)`
- 🚫 扩展降权标识 - 评分函数新增 `3d`/`高燃`/`降调`/`节目`/`氛围`/`慢摇` 等标识降权

### v2.11.3
- 🐛 修复歌词解析失败 - 原 `_parse_lrc` 正则 `\d{2}:\d{2}[.:]\d{2}` 过于严格，1位分钟 `[1:23.45]`、3位毫秒 `[01:23.456]`、无毫秒 `[01:23]`、1位秒 `[01:3.45]` 等常见格式都无法匹配导致解析为空；放宽为 `\d{1,3}:\d{1,2}([.:]\d{1,3})?`，毫秒按位数自动归一化（2位百分位/3位千分位）
- 🐛 修复一行多时间标签解析 - `[01:23.45][02:34.56]歌词` 原正则只解析第一个时间标签且 text 错误包含第二个标签；改为先提取所有时间标签再统一去除标签计算 text
- 🎯 改进自动搜索匹配准确度 - 新增 `_score_candidate` 评分函数，对候选按歌名/歌手与关键词的匹配度排序，原唱优先，伴奏/纯音乐/Cover/翻唱/Remix/Live 等标识降权，减少"歌词与歌曲不对应"问题
- 🎵 保留间奏空行时间锚点 - 原代码 `if text:` 丢弃空歌词行，导致间奏期间歌词显示错乱；现在保留空行作为时间锚点

### v2.11.2
- 🐛 根治终端多行残留 - emoji 在终端占 2 列但 Python `len()` 算 1，导致按字符数截断后实际显示宽度仍超终端宽度触发自动换行，使 ANSI 行清除序列对不上行数；新增 `_display_width`/`_truncate_to_width` 按 emoji/CJK 实际显示宽度计算
- 📊 动态进度条长度 - 根据终端宽度和状态栏内容（含 emoji 宽度）动态调整 bar 长度，保证整行不超宽不换行，时间始终完整显示
- 🔧 播放速度显示格式化 - `1.0999999999999999x` 改为 `1.1x`

### v2.11.1
- 🐛 修复终端残留 - 终端尺寸变化或歌词行从长变短时，旧字符残留问题；改用 ANSI 行计数精确清除上次输出的所有行
- 🎨 歌词加粗显示 - 当前歌词行使用加粗青色（`\033[1;36m`）高亮，与进度条视觉区分
- 🐛 修复 `N` 键手动搜索卡死 - Linux 下终端处于 raw 模式时 `input()` 回车不提交导致卡死；调用前临时恢复终端到正常模式，调用后重新设回 raw
- 🧹 退出播放时自动清除进度显示，避免残留字符

### v2.11.0
- 🌐 新增在线歌词搜索 - 播放时本地无 `.lrc` 自动后台搜索（QQ音乐优先，网易云回退），按 `N` 手动搜索并从候选列表选择
- 🏷️ 搜索关键词构造 - 元数据（title+artist）优先，无元数据回退到清洗后的文件名（去除序号/音质标识/网址/分隔符）
- 🎯 质量过滤 - 仅采用带时间轴的 LRC 歌词，纯文本歌词自动丢弃
- 💾 自动缓存 - 搜索成功后保存为同名 `.lrc`，自动搜索不覆盖本地已存在文件，手动搜索覆盖
- 🎮 新增快捷键 `N`（手动在线搜索歌词，带候选选择交互）
- 🔧 状态栏显示歌词来源标识（📝 歌词(QQ) / 📝 歌词(网易)）
- 🐛 修复已知问题

...
_mp_completion() {
    ...
    # 选项补全
    if [[ "$cur" == -* ]]; then
        COMPREPLY=($(compgen -W "-h --help -i --info -p --playlist -s --shuffle -l --loop -v --volume --speed --favorites --history --clear-history --stats --clear-stats --convert --eq --eq-list --radio --radio-list --radio-add --radio-del --browse --noise --edit-tags --record --extract-audio --to-gif --screenshot --normalize --export-m3u --import-m3u --library-scan --library-search --library-stats --library-clear --trim --merge --reverse --fade --reverb --reverb-list --extract-subtitles --bpm --contact-sheet --find-duplicates --backup-config --restore-config --rename --spectrogram --waveform --cover --gain --ringtone --channels --resample --mux --fade-sec --health-check --silence-detect --silence-cut --silence-threshold --silence-duration --split --export-csv --fingerprint --volume-ramp --ascii-art --ascii-width --ascii-fps --mix --vconcat --scale --rotate --crop --fps --strip-metadata --repeat --fmt --at --count --gif-start --gif-duration --gif-width --gif-fps --rows --cols --recursive --dry-run" -- "$cur"))
        return 0
    fi

    # --fmt 选项补全
    if [[ "$prev" == "--fmt" ]]; then
        COMPREPLY=($(compgen -W "mp3 wav ogg m4a flac aac opus loudnorm dynaudnorm loudnorm_db srt ass" -- "$cur"))
        return 0
    fi

    # --reverb 预设补全
    if [[ "$prev" == "--reverb" ]]; then
        COMPREPLY=($(compgen -W "room hall cathedral plate spring cave stadium" -- "$cur"))
        return 0
    fi

    # --convert 均衡器预设补全
    if [[ "$prev" == "--eq" ]]; then
        COMPREPLY=($(compgen -W "flat rock pop jazz classical bass treble vocal electronic" -- "$cur"))
        return 0
    fi

    # 文件补全（音频/视频文件）
    local files=$(compgen -f -- "$cur" | grep -E '\.(mp3|wav|ogg|m4a|flac|aac|opus|mp4|mkv|avi|mov|webm|m4v|flv|wmv|m4b|srt|ass|m3u|m3u8)$' || true)
